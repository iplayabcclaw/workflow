...
function workflowFields(form, includeFiles = false) {
  ...
  if (!includeFiles) {
    const images = [...form.elements.image.files]; const audios = [...form.elements.audio.files];
    images.forEach((image, index) => { payload[`ref_image_${index}`] = `data:${image.type};base64,<已选文件 ${image.name}，${image.size} bytes>`; });
...
function syncImageInput() { const transfer = new DataTransfer(); selectedImages.forEach(file => transfer.items.add(file)); $('#form').elements.image.files = transfer.files; }
function updateImagePreview() { const files = selectedImages; $('#imagePreview').innerHTML = files.length ? files.map((file, index) => `<figure><button type="button" class="remove-image" data-index="${index}" aria-label="删除 ${esc(file.name)}">×</button><img src="${URL.createObjectURL(file)}" alt="${esc(file.name)}"><figcaption>${esc(file.name)}</figcaption></figure>`).join('') + `<span class="image-count">已选择 ${files.length} 张图片，可继续添加或拖入</span>` : ''; }
function addImages(files) { selectedImages = [...selectedImages, ...files.filter(file => file.type.startsWith('image/'))]; syncImageInput(); updateImagePreview(); updatePreview(); }
...
async function requestBody(form) {
  ...
  if (workflow_type === 'image_audio' && form.elements.image.files.length) payload.image_data_urls = await Promise.all([...form.elements.image.files].map(readDataUrl));
...
function setWorkflow(workflow) {
  ...
  if (isText) { form.elements.seed.value = ''; selectedImages = []; form.elements.image.value = ''; form.elements.audio.value = ''; updateImagePreview(); clearReuseAssets(); }
...
$('#form').elements.image.addEventListener('change', event => { addImages([...event.target.files]); });
$('#imagePreview').addEventListener('click', event => { if (!event.target.matches('.remove-image')) return; selectedImages.splice(Number(event.target.dataset.index), 1); syncImageInput(); updateImagePreview(); updatePreview(); });
$('#referenceFields').addEventListener('dragover', event => { event.preventDefault(); $('#referenceFields').classList.add('drag-over'); });
$('#referenceFields').addEventListener('dragleave', () => $('#referenceFields').classList.remove('drag-over'));
$('#referenceFields').addEventListener('drop', event => { event.preventDefault(); $('#referenceFields').classList.remove('drag-over'); addImages([...event.dataTransfer.files]); });
$('#form').addEventListener('submit', async event => {
  ...
  try { button.disabled = true; $('#notice').textContent = '正在上传并提交…';
    ...
    $('#notice').textContent = `已提交：${result.task_id}`; form.reset(); selectedImages = []; updateImagePreview(); clearReuseAssets();
...
setWorkflow('image_audio'); loadTasks(); setInterval(loadTasks, 60000);
